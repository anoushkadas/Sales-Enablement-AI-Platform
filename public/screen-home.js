...
function buildHomeScreen(conv, onShowToast, userEmail) {
  ...
  function render() {
    ...
    if (showingTest) {
      container.appendChild(buildTopbar("Test mode", "Scenario-based questions, scored as you go"));
      const content = el("div", { className: "content" });
      content.appendChild(window.TestFlow.buildTestFlow(() => { showingTest = false; render(); }));
    ...
    if (hasStarted) {
      ...
      scrollArea.appendChild(
        window.DealPicker.buildDealPicker(conv.state.anchoredDeal, (deal) => { conv.state.anchoredDeal = deal; })
      );
    ...
    const dealPickerWrap = el("div", { style: { marginBottom: "20px", display: "flex", justifyContent: "center" } });
    dealPickerWrap.appendChild(
      window.DealPicker.buildDealPicker(conv.state.anchoredDeal, (deal) => { conv.state.anchoredDeal = deal; })
    );
    content.appendChild(dealPickerWrap);
    ...
    window.APP_DATA.MODES.forEach(m => {
      tileGrid.appendChild(
        el("button", {
          className: "mode-tile",
          onclick: () => {
            if (m.id === "test") { showingTest = true; render(); return; }
            handleSend(`Let's start a ${m.label} session.`);
          },
        }, [
